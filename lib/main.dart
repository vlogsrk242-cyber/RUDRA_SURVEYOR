...
class _AreaPageState extends State<AreaPage> {
  ...
  void calculate() {
    final l = double.tryParse(length.text.trim()) ?? 0;
    final w = double.tryParse(width.text.trim()) ?? 0;

    if (l <= 0 || w <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('કૃપા કરીને લંબાઈ અને પહોળાઈ સાચી નાખો.')),
      );
      return;
    }
    ...
    setState(() {
      area = l * w;
      areaFt2 = area / 0.09290304;
      areaAcre = area / 4046.8564224;
      areaHectare = area / 10000;
      areaGuntha = area / 101.17141056;
    });
  }
  ...
  Widget resultRow(String unit, double value) {
    return Container(
      margin: const EdgeInsets.only(bottom: 10),
      ...
      body: ListView(
        ...
        children: [
          ...
          if (area > 0) ...[
            ...
            resultRow('m²', area),
            resultRow('ft²', areaFt2),
            resultRow('Acre', areaAcre),
            resultRow('Hectare', areaHectare),
            resultRow('Guntha', areaGuntha),
          ],
        ],
      ),
    );
...
class _UnitPageState extends State<UnitPage> {
  ...
  final areaUnits = ['m²', 'ft²', 'Acre', 'Hectare', 'Guntha'];
